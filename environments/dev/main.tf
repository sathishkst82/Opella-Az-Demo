...
module "storage" {
  ...
  network_rules = {
    # GitHub-hosted runners are outside the VNET, so DEV keeps data-plane access open for CI.
    # In production, use a self-hosted runner in the VNET or explicit runner IP allowlists.
